...
设 `现货账户持仓 = spot`，`其他账户合计 = other`，`total = spot + other`，门槛 `REQ = 100`。仅当 **`spot ≥ REQ`（100TX 在指定校验账户）** 才解锁。未达标时当前持仓明细整体**高斯模糊**（不可选中、不可复制），Tab 标题带 🔒 标记，上层覆盖引导层含锁 icon + 持仓要求说明 + 智能按钮：

| 状态 | 条件 | 提示 | 按钮 |
|---|---|---|---|
| 解锁 | spot ≥ 100 | — | 无（清晰展示，Tab 无锁标） |
| 划转即可 | spot < 100 且 total ≥ 100 | 现货账户 spot · 其他账户 other，划转 (100−spot) 个即可解锁 | **【去划转 100TX】** |
| 划转+购买 | spot < 100 且 total < 100 且 other > 0 | 现货 spot · 其他账户 other · 共 total，还需购买 (100−total) 个 | **【去划转 100TX】**（次）+ **【去购买 100TX】**（主）；先划转已有再购买缺口 |
| 购买 | spot < 100 且 other = 0 | 你当前持有 spot | **【去购买 100TX】** |

> 逻辑要点：因门槛锁定"指定账户持仓"，只要 total < 100，划转无法满足，必须购买缺口 → 同时给"划转+购买"两个按钮。
...
**描述**：按**方案 B（指定账户校验）**校验用户在指定账户的 100TX 持仓量，并按状态提供购买 / 划转入口。

**业务规则**
- 校验账户：仅校验**指定账户**（原型示例为「现货账户」）的 100TX 持仓；其他账户（合约/资金/理财等）不直接计入解锁判定，但用于判断是否可划转。[待确认] 具体锁定哪个账户
- 阈值：指定账户持仓 `spot ≥ 100` 即解锁；[待确认] "当前快照"还是"持续持有"。
- 智能按钮逻辑（见 F3 门禁交互四态表）：
  - `total ≥ 100 且 spot < 100` → 【去划转 100TX】（从其他账户划入指定账户）
  - `total < 100 且 other > 0` → 【去划转】+【去购买】（先划转已有，再购买 100−total 缺口）
  - `other = 0 且 spot < 100` → 【去购买 100TX】
...
| 字段 | 类型 | 说明 |
|---|---|---|
| requiredAccount | string | 校验账户，如 spot（现货账户）[待确认] |
| spotAmount | decimal | 指定校验账户的 100TX 持仓量 |
| otherAmount | decimal | 其他账户合计 100TX 持仓量（用于判断可否划转） |
...
### 必须确认（阻塞开发）
1. [待确认] 方案 B 校验账户：具体锁定哪个账户（现货/资金/合约）？"当前快照"还是"持续持有"？划转页是否预填补足数量？→ F5
