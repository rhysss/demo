...
**门禁交互**
- 达标（持有 ≥100 个 100TX）：正常清晰展示，Tab 标题不带锁标。
- 未达标：
  - 当前持仓明细整体**高斯模糊**（不可选中、不可复制）；
  - 上层覆盖引导层：锁 icon + 文案"持有 100 个 100TX 即可实时查看该用户的当前持仓明细" + "持仓要求 100 100TX · 你当前持有 X" + **【去购买 100TX】** 按钮；
  - Tab 标题带 🔒 标记；
  - 文案提示"历史持仓无需持仓即可查看"。
...
**描述**：校验当前登录用户 100TX 持仓量，并提供购买入口。

**业务规则**
- 校验口径：当前钱包可用 + [待确认]（是否含锁仓/理财/质押中的 100TX）。
- 阈值：≥100 个即解锁；[待确认] 是否需要"持续持有"而非"当前快照"。
- 校验时机：进入当前持仓 Tab 时；购买返回时；下拉刷新时。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| holdAmount | decimal | 用户当前 100TX 持仓量 |
| threshold | int | 门槛，=100 |
| unlocked | bool | 是否解锁当前持仓 |
...
### 必须确认（阻塞开发）
1. [待确认] 100TX 持仓校验口径：是否含锁仓/理财/质押中的？"当前快照"还是"持续持有"？→ F5
