...
|---|---|---|---|---|
| 未持有 100TX 的登录用户 | ✅ 免费可见 | ✅ 可看 | ❌ 模糊(遮罩)+引导购买/划转 | ❌ 全隐藏 |
| 持有 ≥100 个 100TX 的登录用户 | ✅ 免费可见 | ✅ 可看 | ✅ 可看 | ❌ 仍隐藏 |
| 未登录游客 | [待确认] 是否允许浏览 | [待确认] | ❌ | ❌ |
| 上榜用户本人 | ✅ | [待确认] 是否可见自己完整数据/是否可关闭被展示 | — | — |

> 注 1：数量与保证金对**所有角色**均不展示，与是否持有 100TX 无关。
> 注 2：**榜单页（含未实现/已实现两榜）本身免费无门禁**；100TX 门禁仅作用于**详情页「当前持仓」**（需资金账户 ≥100 个）。历史持仓免费。
...
- 点击任一卡片 → 进入持仓详情。
- **两个榜单本身均免费可见、无门禁**；100TX 持仓门禁仅在**点击 UID 进入详情页的「当前持仓」Tab** 时触发（见 F3）。
