...
| 角色 | 已实现榜 / 历史持仓 | 未实现榜 / 当前持仓 | 数量/保证金字段 |
|---|---|---|---|
| 未持有 100TX 的登录用户 | ✅ 可看 | ❌ 模糊(遮罩)+引导购买/划转 | ❌ 全隐藏 |
| 持有 ≥100 个 100TX 的登录用户 | ✅ 可看 | ✅ 可看 | ❌ 仍隐藏 |
| 未登录游客 | [待确认] 是否允许浏览 | ❌ | ❌ |
| 上榜用户本人 | [待确认] 是否可见自己完整数据/是否可关闭被展示 | — | — |

> 注 1：数量与保证金对**所有角色**均不展示，与是否持有 100TX 无关。
> 注 2：100TX 门禁按"当前持仓口径"统一生效——**未实现盈亏榜**与**详情页当前持仓**同权限（需资金账户 ≥100 个）；**已实现盈亏榜**与**历史持仓**为历史口径，全员免费。
...
- 点击任一卡片 → 进入持仓详情。
- **榜单级持仓门禁**：未实现盈亏榜展示的是"当前持仓"口径数据，**与详情页当前持仓同权限**——未达标（资金账户 100TX < 100）时整个未实现榜内容**高斯模糊（遮罩）+ 锁 icon + 需要/已有文案 + 去购买/去划转双按钮**，解锁后正常展示并可点击进入详情；**已实现盈亏榜（历史口径）对所有人免费**，无遮罩。Tab 切换不受门禁影响（未解锁也能切到已实现榜）。
