...
| 版本 | 日期 | 修订人 | 备注 |
|---|---|---|---|
| V1.0 | 2026-08-09 | Rhys | 首版，含未实现/已实现双榜入口、持仓详情、100XT 持仓门禁 |
...
同时，平台希望以持仓明细作为 **100XT 代币的核心权益场景**——通过"查看他人实时当前持仓需持有 100 个 100XT"来驱动代币需求与持仓沉淀。

#### 1.1.2 产品概述
在合约盈利榜基础上，为每个榜单用户增加**持仓详情页**，分「当前持仓」与「历史持仓」两个 Tab：
- **历史持仓**：所有用户免费可看；
- **当前持仓**：需持有 **100 个 100XT** 解锁；未达标时明细模糊处理并引导购买 100XT。

持仓明细中，**保证金、数量（及数量类字段）不展示**，以保护上榜者的仓位规模与资金体量，降低被反向狙击/抄底风险。

#### 1.1.3 产品目标
**业务目标**
- 100XT 代币新增持仓地址数 / 人均持仓量提升（核心）；[待确认] 具体目标值
- 榜单页 → 持仓详情页点击转化率 ≥ [待确认]%；
- 持仓详情页 → 100XT 购买页点击转化率 ≥ [待确认]%。
...
- **潜在 100XT 持有者**：对"解锁实时当前持仓"有付费/持币意愿的用户；
...
| 100XT | 平台代币，本功能中作为解锁"当前持仓"的持仓门槛资产 |
| 持仓门禁 | 需持有指定数量 100XT 才能查看当前持仓的准入机制 |
| 数量类字段 | 暴露仓位规模的字段：数量、保证金、最大持仓量、已平仓量等 |

### 1.3 角色及权限

| 角色 | 榜单（两个榜） | 详情·历史持仓 | 详情·当前持仓 | 数量/保证金字段 |
|---|---|---|---|---|
| 未持有 100XT 的登录用户 | ✅ 免费可见 | ✅ 可看 | ❌ 模糊(遮罩)+引导购买/划转 | ❌ 全隐藏 |
| 持有 ≥100 个 100XT 的登录用户 | ✅ 免费可见 | ✅ 可看 | ✅ 可看 | ❌ 仍隐藏 |
| 未登录游客 | [待确认] 是否允许浏览 | [待确认] | ❌ | ❌ |
| 上榜用户本人 | ✅ | [待确认] 是否可见自己完整数据/是否可关闭被展示 | — | — |

> 注 1：数量与保证金对**所有角色**均不展示，与是否持有 100XT 无关。
> 注 2：**榜单页（含未实现/已实现两榜）本身免费无门禁**；100XT 门禁仅作用于**详情页「当前持仓」**（需资金账户 ≥100 个）。历史持仓免费。

### 1.4 文档阅读对象
产品、前端、后端、测试、UED、运营（100XT 增长）。
...
5. 「当前持仓」需持有 100 个 100XT 才可查看；未达标时明细高斯模糊 + 覆盖引导购买。
...
flowchart TD
    ...
    E --> G{是否持有 ≥100 个 100XT?}
    G -->|是| H[清晰展示当前持仓明细]
    G -->|否| I[明细模糊 + 引导购买 100XT]
    I -->|点击去购买| J[100XT 购买页]
    E -.切换Tab.-> K[历史持仓 · 全员可看]
    F -.切换Tab.-> E
    K --> L[按 时间/币种/方向 筛选]
...
stateDiagram-v2
    [*] --> 校验持仓
    校验持仓 --> 已解锁: 持有100XT≥100
    校验持仓 --> 已锁定: 持有100XT<100
    已锁定 --> 校验持仓: 购买100XT后返回/刷新
    已解锁 --> [*]
    已锁定 --> [*]
```
...
### 2.4 产品版本规划
- **V1.0（本期）**：双榜入口点击、持仓详情双 Tab、字段隐藏规则、100XT 持仓门禁、历史筛选。
...
| F3 | 当前持仓 Tab | 字段展示 + 100XT 门禁模糊/引导 | P0 |
| F4 | 历史持仓 Tab | 字段展示 + 时间/币种/方向筛选 | P0 |
| F5 | 100XT 持仓校验与购买跳转 | 校验持仓、跳转购买页 | P0 |
...
- 点击任一卡片 → 进入持仓详情。
- **两个榜单本身均免费可见、无门禁**；100XT 持仓门禁仅在**点击 UID 进入详情页的「当前持仓」Tab** 时触发（见 F3）。

**跳转规则（关键）**

| 来源榜单 | 默认落地 Tab | 说明 |
|---|---|---|
| 未实现盈亏榜 | 当前持仓 | 未实现盈亏对应持有中仓位，落地当前持仓，命中 100XT 转化点 |
...
### F3 当前持仓 Tab（100XT 门禁）

**描述**：展示用户当前持有中的仓位明细，需持有 100 个 100XT 解锁。

**用户故事**：作为持有 100XT 的用户，我希望查看高手当前的开仓/标记/强平价与保证金比率，以便研究其实时仓位。
...
设 `资金账户持仓 = spot`，门槛 `REQ = 100`。仅当 **`spot ≥ REQ`（100XT 在指定校验账户）** 才解锁。

| 状态 | 条件 | 展示 |
|---|---|---|
| 解锁 | spot ≥ 100 | 清晰展示，Tab 无锁标 |
| 未达标 | spot < 100 | 明细整体**高斯模糊**（不可选中/复制），Tab 带 🔒；覆盖引导层含锁 icon + 文案「资金账户需持有 **100 100XT** · 你当前持有 **{spot}**」+ **固定两个按钮：【去购买 100XT】（主）/【去划转 100XT】（次）** |

> 说明：未达标时不做条件分支，统一同时提供"购买 / 划转"两个入口，由用户自行选择（没有 100XT 就购买，其他账户有就划转）。

**异常/分支流程**
- 该用户当前无持仓：展示空状态"该用户暂无当前持仓"（达标用户才可见此结论；未达标仍展示门禁）。[假设]
- 持仓校验接口超时/失败：按未解锁处理并可重试。
- 用户购买 100XT 后返回：需**重新校验持仓并自动解除模糊**（返回即刷新或下拉刷新）。[待确认] 刷新时机
...
**描述**：按**方案 B（指定账户校验）**校验用户在指定账户的 100XT 持仓量，未达标时统一提供购买 / 划转两个入口。

**业务规则**
- 校验账户：仅校验**资金账户**的 100XT 持仓；其他账户（现货/合约/理财等）不计入解锁判定。
- 阈值：指定账户持仓 `spot ≥ 100` 即解锁；[待确认] "当前快照"还是"持续持有"。
- 未达标按钮：固定同时展示 **【去购买 100XT】（主）/【去划转 100XT】（次）**，不做条件分支，由用户自行选择。
- 校验时机：进入当前持仓 Tab 时；购买/划转返回时；下拉刷新时。
- 【去购买 100XT】跳转目标：[待确认] 100XT 现货/兑换购买页 URL。
- 【去划转 100XT】跳转目标：账户划转页，默认「转出=其他账户、转入=指定账户、币种=100XT、数量=建议补足额」。[待确认] 是否预填数量
...
| fundingAmount | decimal | 资金账户的 100XT 持仓量（= 解锁判定依据） |
| otherAmount | decimal | 其他账户合计 100XT 持仓量（供用户判断可否划转） |
...
| 5 | 未持有 100 个 100XT：当前持仓明细模糊、不可复制、展示引导层与"去购买"按钮 |
| 6 | 持有 ≥100 个 100XT：当前持仓清晰展示，Tab 无锁标 |
| 7 | 历史持仓对所有登录用户可见，无需持仓 |
| 8 | 历史筛选（时间/币种/方向）实时生效，选中态高亮，无结果显示空状态 |
| 9 | 购买 100XT 返回后当前持仓自动解锁（无需手动重进） |
